...
import { Input, InputNumber, message, Space, SpaceCompact } from 'antdv-next';
...
function containImage(suffix: string) {
  const suffixArray = new Set(suffix.split(','));
  return suffixArray.has('jpg') || suffixArray.has('jpeg');
}
</script>
<template>
  <Drawer class="w-[600px]" title="文件上传策略">
    <BaseForm>
      ...
      <template #maxSize>
        <SpaceCompact compact>
          <InputNumber v-model:value="maxWidth" :min="1" style="width: 120px" />

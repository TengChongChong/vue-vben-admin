...
const exportConfig = ref<FieldConfig[]>([]);
const columns = [
  {
    title: '导出',
    dataIndex: 'isEnable',
...
async function handleSave() {
  emit('updateConfig', unref(exportConfig));
  modalApi.close();
}
...
const [Modal, modalApi] = useVbenModal({
  onOpenChange: async (isOpen: boolean) => {
    if (isOpen) {
      ...
      exportConfig.value = cloneDeep(config);
    }
  },
});
</script>
<template>
  <Modal class="w-[1200px]" title="导出配置">
    <Table
      :columns="columns"
      :data-source="exportConfig"
      :pagination="false"
      size="small"
    >

...
const importConfig = ref<FieldConfig[]>([]);
const columns = [
  {
    title: '导入',
    dataIndex: 'isEnable',
...
async function handleSave() {
  emit('updateConfig', unref(importConfig));
  modalApi.close();
}
...
const [Modal, modalApi] = useVbenModal({
  onOpenChange: async (isOpen: boolean) => {
    if (isOpen) {
      ...
      importConfig.value = cloneDeep(config);
    }
  },
});
</script>
<template>
  <Modal class="w-[1200px]" title="导入配置">
    <Table
      :columns="columns"
      :data-source="importConfig"
      :pagination="false"
      size="small"
    >

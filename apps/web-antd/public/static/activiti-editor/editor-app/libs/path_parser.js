...
PathParser.prototype.parseData=function(pathData){if(typeof(pathData) !== "string")throw new Error("PathParser.parseData: The first parameter must be a string");if(this._handler!=null&&this._handler.beginParse!=null)this._handler.beginParse();const lexer=this._lexer;lexer.setPathData(pathData);let mode="BOP";var token=lexer.getNextToken();while(!token.typeis(PathToken.EOD)){let param_count;var params=[];switch(token.type){case PathToken.COMMAND:if(mode=="BOP"&&token.text!="M"&&token.text!="m")throw new Error("PathParser.parseData: a path must begin with a moveto command");mode=token.text;param_count=PathParser.PARAMCOUNT[token.text.toUpperCase()];token=lexer.getNextToken();break;case PathToken.NUMBER:break;default:throw new Error(`PathParser.parseData: unrecognized token type: ${token.type}`);}for(let i=0;i<param_count;i++){switch(token.type){case PathToken.COMMAND:throw new Error(`PathParser.parseData: parameter must be a number: ${token.text}`);case PathToken.NUMBER:params[i]=token.text-0;break;default:throw new Errot(`PathParser.parseData: unrecognized token type: ${token.type}`);}token=lexer.getNextToken();}if(this._handler!=null){const handler=this._handler;var method=PathParser.METHODNAME[mode];if(handler[method]!=null)handler[method].apply(handler,params);}if(mode=="M")mode="L";if(mode=="m")mode="l";}};
